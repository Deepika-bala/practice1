...
class New_class:
    ...
    def read_file(self):
        f = open(self.file, 'r')  #pylint: disable=consider-using-with
        crk = f.read().split()
        print(crk)
        return crk
...
class Student(New_class):
    ...
    def pali(self, crk):  #pylint: disable=redefined-outer-name
        ...
        for xc in crk:
            if xc == xc[::-1]:
                self.printstmt("The palindrome is",xc)
                return xc
    ...
    def printstmt(self, data, ans):  #pylint: disable=no-self-use
        print(data, ans)
        strg = data + ":" + str(ans)   #pylint: disable=redefined-outer-name
        logging.info(strg)


unique_filename = str(uuid.uuid4())  #pylint: disable=invalid-name
